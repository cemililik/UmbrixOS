...
- **2026-04-28 — IRQ-armed half is now live ([T-012](../analysis/tasks/phase-b/T-012-exception-and-irq-infrastructure.md), commit `<TBD-commit-3>`).** T-012 implements `<QemuVirtCpu as Timer>::arm_deadline` and `cancel_deadline` against the EL1 virtual generic-timer (PPI 27); see [UNSAFE-2026-0021](../audits/unsafe-log.md#unsafe-2026-0021--el1-virtual-generic-timer-compare-register-writes). The conversion path uses [`tyrne_hal::timer::ns_to_ticks`](../../hal/src/timer.rs) — the inverse of the existing `ticks_to_ns` helper, with the same 128-bit-intermediate / saturating-cast discipline (round-trips against `ticks_to_ns` are exact at frequencies that divide evenly into 1e9 ns/s, with bounded integer-truncation drift at non-divisor frequencies). The `Timer` trait surface this ADR specifies is unchanged; both signatures and the `Timer::resolution_ns` precondition match the original §Decision outcome. The ADR body is unchanged; this rider records that the deferred half is closed and points readers to the audit-log entry that covers the unsafety surface.

  Trait-side details preserved by the implementation:
  - `arm_deadline(deadline_ns: u64)` is **absolute time**, matching the §Decision-outcome semantics. The implementation writes `CNTV_CVAL_EL0` (64-bit absolute compare) rather than `CNTV_TVAL_EL0` (32-bit relative), so the absolute-time guarantee holds across the full `u64` range.
  - The trait method does not enable the GIC line itself; v1's BSP layer composes the timer write with a `gic.enable(TIMER_IRQ)` call (`IrqController` from ADR-0011) inside `arm_deadline`. The composition lives at the BSP-call site rather than inside the trait so test-hal's mock `Timer` impl does not have to know about the GIC — the trait surface stays orthogonal to interrupt-controller concerns, which was an explicit decision driver in §Decision outcome.
  - `cancel_deadline` masks the timer at the source (`CNTV_CTL_EL0 = IMASK`) **and** disables the GIC line. The IRQ-handler dispatch path also masks the timer to prevent the same deadline from re-firing before the next `arm_deadline` re-arm; this is an implementation detail of the BSP and is not expressed in the trait surface.
