...
1. **ADR-0026 — Secondary core start protocol.** PSCI vs. spin-table. Entry point for secondaries (shared with primary or separate). Rendezvous semantics (when the primary considers a secondary "up").
...
1. **ADR-0027 — Per-core state access pattern.** `TPIDR_EL1` pointer vs. indexed lookup. Thread-local-like access to the current core's state.
...
1. **ADR-0028 — Scheduler topology.** Per-core queues with work stealing, vs. a single global queue with locking, vs. hybrid. Real-time guarantees (or the lack thereof).
...
1. **ADR-0029 — Cross-core wakeup.** IPI-based (inter-processor interrupt) vs. polling. Latency expectations.
...
1. **ADR-0030 — TLB shootdown protocol.** Broadcast IPI vs. per-address targeted; whether to extend `Mmu` trait or add a sibling.
...
|-----|---------|----------------|
| ADR-0026 | Secondary core start protocol | C1 |
| ADR-0027 | Per-core state access pattern | C2 |
| ADR-0028 | Scheduler topology (preemptive) | C3 |
| ADR-0029 | Cross-core wakeup (IPI) | C4 |
| ADR-0030 | TLB shootdown protocol | C5 |
