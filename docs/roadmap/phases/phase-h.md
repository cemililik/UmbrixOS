...
1. **ADR-0051 — Pi 5 differences.** RP1 southbridge, peripheral topology, console routing, GIC changes.
...
1. **ADR-0052 — Jetson boot chain.** CBoot / U-Boot sequence, where Umbrix inserts itself.
...
1. **ADR-0053 — RISC-V target choice.** Specific board, specific ISA subset (RV32 vs. RV64, extensions).
...
|-----|---------|----------------|
| ADR-0051 | Pi 5 differences | H1 |
| ADR-0052 | Jetson boot chain | H2 |
| ADR-0053 | RISC-V target choice | H3 |

## Open questions carried into Phase H

- Whether any HAL trait needs a v2 to accommodate architectural differences that Phase 4b could not foresee.
