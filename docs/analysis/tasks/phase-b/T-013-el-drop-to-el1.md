...
- **Tests.**
  - [x] QEMU smoke at default config (EL1 entry) — boot trace unchanged. *(Verified by maintainer 2026-04-27 prior to PR #9 merge; trace matches the A6 + T-009 instrumentation lines.)*
  - [x] QEMU smoke at `-machine virtualization=on` (EL2 entry) — boot trace identical (proves EL drop landed correctly). *(Verified by maintainer 2026-04-27; same trace as the default-config run, confirming the EL2 → EL1 transition lands at `post_eret` cleanly.)*
...
- [x] QEMU smoke reproduces the eight-line trace at default config; same trace at `-machine virtualization=on`. *(Verified by maintainer 2026-04-27 prior to PR #9 merge.)*
...
| 2026-04-27 | @cemililik (+ independent review agent) | Promoted `In Review → Done` via PR #9 merge to `main` (merge commit `9a66e8b`). The previously-deferred QEMU smoke gates were verified by the maintainer prior to merge: both default-config (EL1 entry) and `-machine virtualization=on` (EL2 entry) reproduce the unchanged A6 + T-009 instrumentation trace, confirming the EL drop's correctness on both boot paths. Two independent review-pass rounds produced 14 + 5 + 5 = 24 findings; the most substantive surface T-013 generated was the audit-log discipline thread — UNSAFE-2026-0017 + 0018 were committed in `f289d4d` and subsequently corrected via three separate Amendment blocks (HCR_EL2 literal-write rationale; GAS halt-loop syntax typos; cfg-gating prose tightening) plus a "Discipline note for future readers" paragraph that explicitly documents the introducing-commit boundary as the body-locking event. T-013 is the first ADR-0024-implementing task and successfully closes ADR-0022's first-rider sub-rider's *time-source vs IRQ-delivery* split — the time-source (T-009) and EL drop (T-013) halves are now done; the IRQ-delivery + WFI-activation half remains in T-012. The `boot.s` source is roughly 100 lines of disciplined assembly with one new audit entry covering the K3-12 mask + EL2→EL1 transition; UNSAFE-2026-0016 stays load-bearing as the post-condition assertion. |
