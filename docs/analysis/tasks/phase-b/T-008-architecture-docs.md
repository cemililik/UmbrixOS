...
| 2026-04-27 | @cemililik (+ independent review agent) | Promoted `In Review → Done` via PR #9 merge to `main` (merge commit `9a66e8b`). Cross-reference linting was the main review surface for T-008: the second-pass review caught four broken cross-references in `ipc.md` (T-003 path × 3, `bsp-qemu-virt/src/lib.rs` → `main.rs`) and one in this file (T-004 path) — all fixed in `9a8e312` / `39dd978`. The bidirectional ADR ↔ architecture-doc cross-reference DoD item was the last open AC and was closed in `9680e41` with five short §Revision notes riders to ADR-0010 / 0017 / 0019 / 0020 / 0022 (ADR-0021 already had a direct citation). Two Mermaid diagrams in `scheduler.md` and three in `ipc.md` — all rendered clean across the review. The `boot.md` update T-008's Out-of-scope deferred to T-013 was indeed delivered there. No code changes; no new ADR; documentation-style invariants held. |
